...
(function($){
	var timer,
	defaults={
		class_menuitem:'imc_menuitem',
		class_title:'imc_menutitle',
		class_separater:'imc_separater',
		class_nothing:'imc_nothing',
		class_hover:'imc_hover',
		timeout:800
	},
	options=$.extend({},defaults);
	$.contextMenu=function(_options){
	...
	$.extend($.contextMenu,{
		title:function(key){
			key=key||'';
			return $('<li/>').addClass(options.class_title).text(key)
		},
		separator:function(){return $('<li/>').addClass(options.class_separater)},
		nothing:function(key){key=key||'';return $('<li/>').addClass(options.class_nothing).text(key)}});
	$.fn.contextMenu=function(menu,live){
		if(live){
			this.live('click',collback)
		}else{
			this.on('click',collback)
		}
		return this;
		function collback(e){
			show.call(this,menu,e);
			return false
		}
	};
	function show(menu,e){
		var x=e.pageX,y=e.pageY,menuContainer,containerRight,containerBottom,documentRight,documentBottom;
		hide();
		if(typeof(menu)=='function'){
			menu=menu.call(this,e)
		}
		console.log( menu );
		if(menu==null||menu.length==0){
			return
		}
		menuContainer=createMenuList.call(this,menu,e);
		if(!menuContainer){
			return
		}
		menuContainer.attr('id','imi_contextmenu').css({left:x,top:y}).appendTo('BODY');
		containerRight=menuContainer.offset().left+menuContainer.width()+10;
		containerBottom=menuContainer.offset().top+menuContainer.height()+10;
		documentRight=$(document).scrollLeft()+$(window).width();
		documentBottom=$(document).scrollTop()+$(window).height();
		if(containerRight>documentRight){
			x=x-(containerRight-documentRight);
			menuContainer.css({left:x})
		}
		if(containerBottom>documentBottom){
			y=y-(containerBottom-documentBottom);
			menuContainer.css({top:y})
		}
		(function(container){
			var self=arguments.callee;
			if(container.width()+10>documentRight-container.offset().left){
				container.css({left:-container.width()-2})
			}
			if(container.height()+10>documentBottom-container.offset().top){
				container.css({marginTop:-container.height()+13})
			}
			container.find('> LI > .imc_submenu').each(function(){
				self.call(self,$(this))
			})
		})(menuContainer)
	}
	function hide(){
		var $menu=$('#imi_contextmenu');
		if($menu.length==0){
			return
		}
		$menu.remove();
	...
	function createMenuList(menu,e,sub){
		var itemlist=[],$menu;
		for(key in menu){
			var menuitem=createMenuItem.call(this,key,menu[key],e);
			itemlist.push(menuitem.get(0))
		}
		if(itemlist.length==0){
			return null
		}
		$menu=$('<ul class="imc_menulist"/>').append(itemlist);
		if(sub){
			$menu.addClass('imc_submenu');
		}
		return $menu
	}
	function createMenuItem(key,menuitem,e){
		var self=this;
		if(menuitem===null||menuitem===undefined){
			return $.contextMenu.nothing(key)
		}
		else if(menuitem===$.contextMenu.title){
			return menuitem.call(self,key)
		}
		else if(menuitem===$.contextMenu.separator){
			return menuitem.call(self)
		}
		else if(menuitem===$.contextMenu.nothing){
			return menuitem.call(self,key)
		}
		else if(typeof(menuitem)=='string'){
			return $('<li/>').addClass(options.class_menuitem).text(menuitem)
		}
		else if(typeof(menuitem)=='function'){
			return $('<li/>').addClass(options.class_menuitem).text(key).click(function(){
				hide();
				menuitem.call(self,e)
			})
		}
			...
			var submenu=createMenuList.call(this,menuitem,e,true);
			return $('<li/>')
			.addClass(options.class_menuitem)
			.append(submenu).append(key+'<span class="imc_submenu_mark">‣</span>')
			.click( function() {
				$('.imc_active').not( submenu.parents() ).removeClass('imc_active');
				submenu.addClass( 'imc_active' );
				return false;
			} );
		}
	}
	...
	.on('click',hide)
	.on('contextmenu',hide)
	.on('click','#imi_contextmenu',function(){
		return false
	})
	// .on('mouseenter','#imi_contextmenu',function(){
	// 	if(timer){
	// 		window.clearTimeout(timer);
	// 		timer=null
	// 	}
	// })
	// .on('mouseleave','#imi_contextmenu',function(){
	// 	if(options.timeout>0){
	// 		timer=window.setTimeout(hide,options.timeout)
	// 	}
	// })
})(jQuery);
//. autoPager
...
'.imc_menuitem > .imc_submenu.imc_active { visibility: visible; }' +
...
//		{ title: 'スペシャル戦国くじ', action: '/senkuji/senkuji.php?ex=1' },
	...
	target.find('A').removeAttr( 'href' );
		...
		.append( submenu )
		.click( function() {
			// submenuが表示されているときは全部隠す
			if( submenu.find(':visible').length > 0 ) {
				$('.imc_pulldown').hide();
			}
			// submenuが表示されていなければ自分だけ表示
			else {
				$('.imc_pulldown').hide();
				submenu.show();
			}
		...
		switch ( type ) {
			...
			case '308':
				return '<div class="imc_command_button imc_develop imc_quick" data-type="308">開拓</div>';
					// '<div class="imc_command_sub imc_sub1 imc_quick">即出陣</div></div>';
			case '320':
				return '<div class="imc_command_button imc_meeting imc_new_tab" data-type="320">合流</div>';
...
main: function() {
	...
	// areaの編集(Map.setup()で属性が使われるので最後に実施)
	$('area').each( function( idx, elm ) {
		$(elm).attr( 'onclick', $(elm).attr('onmouseover') );
		$(elm).removeAttr('onmouseout').removeAttr('onmouseover');
		// @Todo: クリックイベントにハイライトアクションを追加 ... まぁそのうち
		// function highlight() {
		// クラス名はonclickの引数
		// <img alt="" class="mapAll01" src="http://cache.sengokuixa.jp/world/20140804-01/img/panel/camp_r_s.png">
		// 	'.mapAll\d'
		// 			img_path = element.src;
		// 			target_width = element.width;
		// 			target_height = element.height;
		// 	var o_ig_mapsAll = document.getElementById("ig_mapsAll");
		// 	var o_ig_mapbox_container = document.getElementById("ig_mapbox_container");
		// 	// 座標のオフセット値の計算
		// 	offset_left = o_ig_mapbox_container.offsetLeft + o_ig_mapsAll.offsetLeft;
		// 	offset_top = o_ig_mapbox_container.offsetTop + o_ig_mapsAll.offsetTop;
			
		// 	var left = target_left + offset_left;
		// 	var top = target_top + offset_top;

		// // 暗転要素の操作
		// var oOverlay = document.getElementById("mapOverlay");
		// oOverlay.style.cssText="display:block;";

		// //ユニットをハイライトさせるための処理
		// popup_unit_image(img_path, top , left ,target_width ,target_height);
		// }

		// mouseoutに相当する挙動を追加 .. Click時のイメージが消えないので
	});
},
